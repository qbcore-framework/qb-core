...
QBCore.Commands.Add('org', 'Check Your Organization', {}, false, function(source)
    local src = source
    local PlayerOrg = QBCore.Functions.GetPlayer(source).PlayerData.org
    TriggerClientEvent('QBCore:Notify', src, Lang:t('info.org_info', {value = PlayerOrg.label, value2 = PlayerOrg.grade.name}))
end, 'user')

QBCore.Commands.Add('setorg', 'Set A Players org (Admin Only)', { { name = 'id', help = 'Player ID' }, { name = 'org', help = 'Name of an organisation' }, { name = 'grade', help = 'Grade' } }, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
        Player.Functions.SetOrg(tostring(args[2]), tonumber(args[3]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.not_online'), 'error')
    end
end, 'god')
