    ...
    if src then
        if citizenid then
            ...
            if PlayerData and license == PlayerData.license then
                ...
                if PlayerData.org then
                    PlayerData.org = json.decode(PlayerData.org)
                else
                    PlayerData.org = {}
                end
...
function QBCore.Player.CreatePlayer(PlayerData)
    ...
    self.Functions.SetOrg = function(org, grade)
        local org = org:lower()
        local grade = tostring(grade) or '0'

        if QBCore.Shared.Orgs[org] then
            self.PlayerData.org.name = org
            self.PlayerData.org.label = QBCore.Shared.Orgs[org].label
            if QBCore.Shared.Orgs[org].grades[grade] then
                local orggrade = QBCore.Shared.Orgs[org].grades[grade]
                self.PlayerData.org.grade = {}
                self.PlayerData.org.grade.name = orggrade.name
                self.PlayerData.org.grade.level = tonumber(grade)
                self.PlayerData.org.isboss = orggrade.isboss or false
            else
                self.PlayerData.org.grade = {}
                self.PlayerData.org.grade.name = 'No Grades'
                self.PlayerData.org.grade.level = 0
                self.PlayerData.org.isboss = false
            end

            self.Functions.UpdatePlayerData()
            TriggerClientEvent('QBCore:Client:OnOrgUpdate', self.PlayerData.source, self.PlayerData.org)
            return true
        end
        return false
    end
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        MySQL.Async.insert('INSERT INTO players (citizenid, cid, license, name, money, charinfo, job, gang, org, position, metadata) VALUES (:citizenid, :cid, :license, :name, :money, :charinfo, :job, :gang, :org, :position, :metadata) ON DUPLICATE KEY UPDATE cid = :cid, name = :name, money = :money, charinfo = :charinfo, job = :job, gang = :gang, org = :org, position = :position, metadata = :metadata', {
            ...
            org = json.encode(PlayerData.org),
...
function QBCore.Functions.CreateAccountNumber()
    ...
    while not UniqueFound do
        AccountNumber = 'US0' .. math.random(1, 9) .. math.random(00000, 99999)
...
function QBCore.Functions.CreatePhoneNumber()
    ...
    while not UniqueFound do
        PhoneNumber = 555 .. math.random(00000,99999)
...
function QBCore.Player.CreateWalletId()
    ...
    local WalletId = nil
    while not UniqueFound do
        WalletId = math.random(000000, 999999)
...
function QBCore.Player.CreateSerialNumber()
    ...
    while not UniqueFound do
        SerialNumber = math.random(00000000, 999999)
